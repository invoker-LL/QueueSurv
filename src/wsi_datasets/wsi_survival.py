...
class WSISurvivalDataset(Dataset):
    ...
    def set_feat_paths_in_df(self):
        ...
        try:
            ...
            # del them
            for miss_feat in missing_feats_in_split: self.data_df = self.data_df.drop(self.data_df[self.data_df['slide_id'] == miss_feat].index)
            # sys.exit()
    ...
    def __getitem__(self, idx):
        ...
        for feat_path in feat_paths:
            if self.use_h5:
                ...
                features_coords = torch.load(feat_path)
                # print(features_coords.keys())
                # features = torch.cat(features_coords['quantize'])
                features = torch.cat(features_coords['features'])
                all_coords += features_coords['coords']
            # import pdb;pdb.set_trace()
        ...
        if len(all_coords) > 0:
            all_coords = torch.from_numpy(np.concatenate(all_coords,axis=0))
