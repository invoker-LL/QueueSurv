...
def main(args):
    ...
    if args.statistic == '1':
        ...
        for i in range(5):
            while True:
                try:
                    load_path = os.path.join(json_dir, f'test_res_{i}.json')
                    break
                except:
                    pass
            # 打开 JSON 文件以读取
            with open(load_path, 'r') as json_file:
                # 使用 json.load() 将 JSON 数据加载为 Python 对象
                data = json.load(json_file)
                c_index_list.append(data['test']['c_index'])
                # pdb.set_trace()
        rounded_list = [round(x, 3) for x in c_index_list]
        print(rounded_list)
        c_index_list = np.asarray(c_index_list)
        mean_value = c_index_list.mean()
        std_value = c_index_list.std()
        print(f"{mean_value:.3f} {std_value:.3f}")  # 输出：0.851 0.049
...
parser.add_argument('--statistic', default='',)
