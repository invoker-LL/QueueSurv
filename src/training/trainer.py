...
def train(datasets, args, mode='classification'):
    ...
    if args.model_type in PROTO_MODELS:
        ...
        for k, loader in datasets.items():
            ...
            if 'LinearEmb' in args.emb_model_type:
                ...
                loader.dataset.X = torch.cat(
                    [torch.Tensor(prob).unsqueeze(dim=-1), torch.Tensor(mean), torch.Tensor(cov)], dim=-1)
    ...
    for epoch in range(args.max_epochs):
        ...
        ### Validation Loop (Optional) # eval every ep
    ...
    writer.close()
    json_dir = args.results_dir.split('/k=')[0]
    save_path = os.path.join(json_dir, f'test_res_{args.split_k}.json')
    import json
    with open(save_path, 'w') as json_file: json.dump(results, json_file, indent=4)
...
def train_loop_survival(model, loader, optimizer, lr_scheduler, loss_fn=None, in_dropout=0.0, print_every=50,
    ...
    for batch_idx, batch in enumerate(loader):
        ...
        if out['loss'] is None or out['loss']==0:
            continue
        ...
        # gc_counter += 1
        # if (batch_idx + 1) % accum_steps==0 and loss != 0:
        #     try:
        ...
        # import pdb;pdb.set_trace()
        all_risk_scores.append(out['risk'][-1:].detach().cpu().numpy())
...
def validate_survival(model, loader,
    ...
    for batch_idx, batch in enumerate(loader):
        ...
        model_kwargs = {'attn_mask': attn_mask, 'label': label, 'censorship': censorship, 'loss_fn': loss_fn,
                        'coords': batch['coords']}
        # with torch.autocast(device_type="cuda"):
            # out, log_dict = model([data, batch['coords']], model_kwargs)
        out, log_dict = model(data, model_kwargs)
