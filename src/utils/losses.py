...
def partial_ll_loss(lrisks, survival_times, event_indicators):
    ...
    survival_times = survival_times[sindex] # sorted via survival time
    event_indicators = event_indicators[sindex]

    queue_weight = torch.exp(-torch.arange(survival_times.shape[0])*0.1).to(sindex.device)[sindex]
    queue_weight = None
    ...
    if queue_weight is not None:
        logL = -torch.sum(uncensored_likelihood*queue_weight)
    else:
